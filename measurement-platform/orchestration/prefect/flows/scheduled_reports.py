...
def _rest_query(table: str, params: str = "") -> list | None:
    ...
    Returns None on request/auth/parse failure so callers can fail closed.
    """
    key = SUPABASE_KEY
    if not key:
        print("Supabase REST query skipped: SUPABASE_SERVICE_KEY is not set")
        return None
    ...
    if result.returncode != 0:
        print(f"Supabase REST query failed for {table}: {result.stderr.strip()}")
        return None
    try:
        data = json.loads(result.stdout)
        if isinstance(data, list):
            return data
        print(f"Supabase REST query returned non-list JSON for {table}")
        return None
    except Exception as e:
        print(f"Supabase REST response parse failed for {table}: {e}")
        return None
