...
def _rest_query(table: str, params: str = "") -> list | None:
    ...
    Returns:
      ...
      - None: query/auth/network failure
    """
    key = SUPABASE_KEY
    if not key:
        print("Supabase REST query skipped: SUPABASE_SERVICE_KEY not set")
        return None
    ...
    if result.returncode != 0:
        print(f"Supabase REST query failed for {table}: curl exit code {result.returncode}")
        return None
    try:
        data = json.loads(result.stdout)
    except Exception:
        print(f"Supabase REST query failed for {table}: invalid JSON response")
        return None

    if isinstance(data, dict):
        # Supabase errors are typically objects with message/code/detail fields.
        msg = data.get("message") if isinstance(data, dict) else None
        print(f"Supabase REST query failed for {table}: {msg or data}")
        return None

    if not isinstance(data, list):
        print(f"Supabase REST query failed for {table}: unexpected response type {type(data).__name__}")
        return None
