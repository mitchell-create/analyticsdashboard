...
def _rest_query(table: str, params: str = "") -> list | None:
    ...
    Returns:
      - list: parsed rows (empty list means successful query with no rows)
      - None: query failed (network/auth/JSON/API error)
    """
    key = SUPABASE_KEY
    if not key:
        return None
    ...
    if result.returncode != 0:
        print(f"REST query failed for {table}: curl exit code {result.returncode}")
        return None
    ...
    try:
        ...
        if isinstance(data, list):
            return data
        # PostgREST returns JSON objects for errors; treat as failures.
        return None
    except Exception:
        return None
