...
Cards like Orders, Revenue, Spend should show SUM for the date range, not daily values.
"""
...
def main() -> int:
    ...
    print("Updating metrics to show totals instead of per-day values...\n")
    ...
    for dashcard in dashcards:
        ...
        # Check if card has client_slug filter
        has_client_slug = "client_slug" in current_sql.lower()
        
        # Build base template tags
        if has_client_slug and "client_slug" in existing_tags:
            template_tags = existing_tags.copy()
        elif has_client_slug:
            template_tags = {
                "client_slug": {
...
"""
            if update_card(headers, card_id, new_sql, template_tags):
                print(f"  ✓ Updated to show total revenue")
                updated += 1
...
"""
            if update_card(headers, card_id, new_sql, template_tags):
                print(f"  ✓ Updated to show total spend")
                updated += 1
...
"""
            if update_card(headers, card_id, new_sql, template_tags):
                print(f"  ✓ Updated to show total impressions")
                updated += 1
...
"""
            if update_card(headers, card_id, new_sql, template_tags):
                print(f"  ✓ Updated to show total clicks")
                updated += 1

    print(f"\n✓ Updated {updated} metric cards to show totals")
    print(f"\n  -> {METABASE_URL}/dashboard/{dashboard['id']}")
    print("\nCards now show total values for the selected date range instead of per-day averages.")
    return 0
