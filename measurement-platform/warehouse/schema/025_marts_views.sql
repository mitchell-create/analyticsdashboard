...
-- Replace only empty placeholder tables. Never use CASCADE here: if one of these
-- public objects has real rows or dependents, abort instead of deleting data.
DO $$
DECLARE
    rel_kind "char";
    has_rows boolean;
BEGIN
    ...
    ELSIF rel_kind IS NOT NULL THEN
        RAISE EXCEPTION 'Refusing to replace unsupported object public.fact_spend_daily';
    END IF;
END $$;
...
BEGIN
    ...
    ELSIF rel_kind IS NOT NULL THEN
        RAISE EXCEPTION 'Refusing to replace unsupported object public.fact_kpi_daily';
    END IF;
END $$;
