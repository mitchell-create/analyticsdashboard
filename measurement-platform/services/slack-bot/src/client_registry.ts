 ...
 * Falls back to single-client mode using SUPABASE_DB_URL when no registry exists.
 */
...
export function loadClientRegistry(): void {
  ...
  // Fallback: single-client mode from SUPABASE_DB_URL
  ...
  if (dbUrl) {
    ...
    console.log("  Client registry: single-client mode from SUPABASE_DB_URL");
  }
}
