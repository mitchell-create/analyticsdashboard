...
import { runReadOnlyQuery, runWriteQuery, logQueryAudit } from "./db";
...
async function queueExperiment(params: ParsedRunCommand, clientSlug?: string): Promise<{ id: number } | string> {
  ...
  const sql = `
    ...
    RETURNING id
  `;
  const { rows, error } = await runWriteQuery(
    sql,
    [params.slug, "geolift", params.startDate, params.endDate, config, "queued"],
    clientSlug,
  );
  if (error) return `Failed to queue experiment: ${error.message}`;
  if (!rows.length) return "Failed to queue experiment: no row returned";
...
async function runAnalysis(
  ...
  const { rows, error } = await runWriteQuery(
    upsertSql,
    [params.experimentSlug, "geolift", dataStart, dataEnd, config, "running"],
    options.clientSlug,
  );
  if (error) return `Database error: ${error.message}`;
  if (!rows.length) return "Database error: no row returned from upsert";
  const experimentId = (rows[0] as { id: number }).id;
